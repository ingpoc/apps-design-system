...
You are applying the **Autonomous Agent Builder design system v0.4** to the `autonomous-agent-builder` codebase. This skill bundles the locked tokens, themes, primitive APIs, page patterns, and status language into a deterministic playbook.
...
| `components.md` | Primitive component API contracts (`StatusPill`, `Surface`, `Eyebrow`, `Tabs`, `Button`, `StatusDot`, `Code`, `Kbd`, `Meter`, `Stat`, `Input`, `BrandMark`). |
| `patterns.md` | Page-level patterns — page intro, three-column chrome, condense-on-scroll, settings drawer, command palette, status pill rules. |
| `status-language.md` | Status taxonomy and tone mapping. Use these strings in copy and the matching tones in UI. |
...
3. **Always use the primitives** in `components.md` for buttons, surfaces, tabs, dots, pills, eyebrows, code, kbd. Never hand-roll equivalents.
...
- All statuses come from `status-language.md`
